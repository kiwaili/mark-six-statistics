...
- **實時進度顯示**: 使用 Server-Sent Events (SSE) 技術實時顯示驗證進度
  - 顯示權重測試進度（測試多組初始權重配置）
  - 顯示驗證期數進度（已處理期數/總期數）
  - 顯示重試次數和狀態
  - 顯示最終預測生成狀態
...
- **實時進度顯示**: 在迭代驗證過程中顯示實時進度
  - 進度條視覺化顯示驗證進度百分比
  - 詳細進度文本顯示當前階段和處理狀態
  - 使用 Server-Sent Events (SSE) 技術實現實時更新
  - 支援長時間運行的驗證過程，用戶可隨時了解進度
...
- **Server-Sent Events (SSE)**: 用於實時推送驗證進度更新

### 前端技術
- **原生 HTML/CSS/JavaScript**: 無需額外框架，輕量高效
- **響應式 CSS**: 使用 Flexbox 和 Grid 布局
- **漸層設計**: 現代化的視覺效果
- **Fetch API + ReadableStream**: 用於接收 SSE 數據流，實現實時進度顯示
...
**響應格式：**
此端點使用 **Server-Sent Events (SSE)** 技術，返回 `text/event-stream` 格式的數據流，實時推送進度更新。

**SSE 事件類型：**
- `type: "start"`: 驗證開始
- `type: "progress"`: 進度更新，包含以下字段：
  - `progress`: 進度百分比 (0-100)
  - `stage`: 當前階段 (`weightTesting`, `validating`, `retrying`, `validationComplete`, `generatingPrediction`)
  - `message`: 進度消息
  - `processedPeriods`: 已處理期數（驗證階段）
  - `totalPeriods`: 總期數（驗證階段）
  - `retryCount`: 當前重試次數
  - `maxRetries`: 最大重試次數
  - `weightSetIndex`: 當前測試的權重配置索引（權重測試階段）
  - `totalWeightSets`: 總權重配置數（權重測試階段）
- `type: "complete"`: 驗證完成
- `type: "result"`: 最終驗證結果（包含完整的驗證數據）
- `type: "error"`: 錯誤信息

**功能特點：**
- **實時進度顯示**: 使用 SSE 技術實時推送驗證進度，用戶可隨時了解當前狀態
- **自動重試機制**: 如果平均每期命中數未達到3或以上，自動重試最多50次
...
- **神經網絡訓練**: 使用多層感知器（2個隱藏層：64和32個神經元）學習模式
