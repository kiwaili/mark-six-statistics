...
function calculateFibonacciScore(allNumbers, excludePeriodNumbers = null, filteredNumbers = null) {
  ...
  for (let num = 1; num <= 49; num++) {
    ...
    // 1. 檢查號碼本身是否為斐波那契數（增強權重）
    if (fibonacciSet.has(num)) {
      score += 30; // 增加斐波那契數本身的權重
      strongSignals++;
    }
    ...
    if (appearances.length > 1) {
      ...
      gaps.forEach(gap => {
        fibonacciSequence.forEach(fib => {
          ...
          if (diff <= 2) { // 允許2期的誤差
            const matchScore = (3 - diff) * 15; // 大幅增加分數
            score += matchScore;
            gapMatches++;
            if (diff === 0) {
    ...
    // 7. 強信號加成（如果有多個強信號，大幅增加分數）
    if (strongSignals >= 3) {
      score += 50; // 多個強信號組合，給予大幅加分
...
function analyzeNumbers(results, weights = {}, excludePeriodNumbers = null) {
  ...
  // 預設權重: 頻率: 10%, 加權頻率: 14%, 間隔: 14%, 模式: 8%, 分布: 14%, 趨勢: 12%, 卡方: 4%, 泊松: 4%, 斐波那契: 20%
  // 增加斐波那契權重以增強其影響力
  // 如果提供了自訂權重，則使用自訂權重
